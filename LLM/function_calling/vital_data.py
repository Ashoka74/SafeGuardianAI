...
def update_victim_json(new_infos: Optional[Dict[str, Any]]):
    ...
    prompt = f"Update the JSON structure: {victim_template}\n\n with accurate informations based on history: {history_infos}\n\n and new informations: {new_infos}\n\n. Output should be a JSON file. Fit new information in the main structure of the template [{victim_template.keys()}]. Leave blank (e.g.""), when there is no information. Do not overwrite existing information provided, unless it's to update it into something more informative. NEVER replace existing information with blank values! Output:"
